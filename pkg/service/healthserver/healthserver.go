...
// healthServer is a running HTTP health server for a certain service
type healthServer interface {
	updateService(*Service)
	shutdown()
}

// healthServerFactory creates a new HTTP health server, used for mocking
type healthServerFactory interface {
	newHealthServer(port uint16, svc *Service) healthServer
}
...
type ServiceHealthServer struct {
	healthServerByPort  map[uint16]healthServer
	portRefCount        counter.IntCounter
	portByServiceID     map[uint16]uint16
	healthServerFactory healthServerFactory
}
...
func New() *ServiceHealthServer {
	return WithHealthServerFactory(&httpHealthServerFactory{})
}

// WithHealthServerFactory creates a new health server with a specific health
// server factory for testing purposes.
func WithHealthServerFactory(healthServerFactory healthServerFactory) *ServiceHealthServer {
...
func (s *ServiceHealthServer) UpsertService(svcID uint16, ns, name string, localEndpoints int, port uint16) {
	...
	if !foundSvc {
		...
		if s.portRefCount.Add(int(port)) {
			s.healthServerByPort[port] = s.healthServerFactory.newHealthServer(port, svc)
		}
	}

	srv, foundSrv := s.healthServerByPort[port]
	if !foundSrv {
...
// Access to this method is not synchronized. It is the caller's responsibility
// to ensure this method is called in a thread-safe manner.
func (s *ServiceHealthServer) DeleteService(svcID uint16) {
...
type httpHealthServerFactory struct{}

func (h *httpHealthServerFactory) newHealthServer(port uint16, svc *Service) healthServer {
