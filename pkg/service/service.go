...
type healthServer interface {
	UpsertService(svcID uint16, svcNS, svcName string, localEndpoints int, port uint16)
	DeleteService(svcID uint16)
}
...
func (s *Service) UpsertService(
	...
	s.healthServer.UpsertService(uint16(svc.frontend.ID), svc.svcNamespace, svc.svcName,
...
func (s *Service) deleteServiceLocked(svc *svcInfo) error {
	...
	s.healthServer.DeleteService(uint16(svc.frontend.ID))
